...
struct LineView: View {
    ...
    let baseLineWidth: CGFloat = 1.0 // Thinner for elegance
    ...
    init(startPoint: CGPoint, endPoint: CGPoint, touchLocation: CGPoint?, area: CGPoint?, duration: Date?, maxEffectRadius: CGFloat) {
        ...
        self._opacity = State(initialValue: Double.random(in: 0.2...0.6))
    }
    ...
    private func computeEffect() -> DentEffect {
        ...
        if let releasePoint = area, let releaseTimeValue = duration {
            let closestPointToRelease = closestPointOnLineSegment(from: releasePoint, toLineSegmentStart: initialStartPoint, end: initialEndPoint)
            ...
            if distanceToLine < maxEffectRadius {
                ...
                if timeSinceRelease <= returnDuration {
                    ...
                    return DentEffect(thicknessScale: 0.4 + 0.6 * easedProgress, displacement: displacement * distanceFactor)
                }
                ...
                if timeSinceRelease > oscillationStartTime && timeSinceRelease <= oscillationEndTime {
                    ...
                    return DentEffect(thicknessScale: 1.0 + abs(combinedEffect) * 0.2, displacement: displacement * distanceFactor)
                }
            }
        }
        return DentEffect()
    ...
    var body: some View {
        TimelineView(.animation) { _ in
            ...
            Path { path in
                ...
                if effect.displacement != 0 {
                    ...
                    if length > 0 {
                        ...
                        let controlPoint = CGPoint(x: midX + effect.displacement * normPerpX, y: midY + effect.displacement * normPerpY)
            ...
            .stroke(.white, lineWidth: baseLineWidth * effect.thicknessScale)
            .opacity(opacity)
            .shadow(color: .white.opacity(0.8), radius: effect.displacement != 0 ? 5 : 0) // Glow when active
        }
        .onAppear {
