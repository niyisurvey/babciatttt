//
//  WeatherAPIKeyCardView.swift
//  BabciaTobiasz
//
//  Settings UI for the OpenWeather API key.
//

import SwiftUI

struct WeatherAPIKeyCardView: View {
    @State private var apiKey: String = OpenWeatherSecrets.apiKey() ?? ""
    ...
    var body: some View {
        GlassCardView {
            VStack(alignment: .leading, spacing: 12) {
                ...
                SecureField("Enter OPENWEATHERMAP_API_KEY", text: $apiKey)
                    .dsFont(.body)
    ...
    private func saveKey() -> Bool {
        ...
        apiKey = trimmed
        let saved = OpenWeatherKeychain.save(trimmed)
        statusMessage = saved ? "Saved to Keychain." : "Save failed."
        return saved
    }

    private func clearKey() {
        let removed = OpenWeatherKeychain.delete()
        apiKey = ""
    ...
    private func runTest() async {
        ...
        testMessage = nil
        do {
            let service = WeatherService(locationService: LocationService())
